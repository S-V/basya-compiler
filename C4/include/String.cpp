...
void String::Copynz( char *dest, const char *src, int destsize ) {
	if ( !src ) {
		common->Warning( "String::Copynz: NULL src" );
		return;
	}
	if ( destsize < 1 ) {
		common->Warning( "String::Copynz: destsize < 1" ); 
		return;
	}
...
void String::Append( char *dest, int size, const char *src ) {
	...
	l1 = strlen( dest );
	if ( l1 >= size ) {
		common->Error( "String::Append: already overflowed" );
	}
...
int String::snPrintf( char *dest, int size, const char *fmt, ...) {
	...
	if ( len >= sizeof( buffer ) ) {
		common->Error( "String::snPrintf: overflowed buffer" );
	}
	if ( len >= size ) {
		common->Warning( "String::snPrintf: overflow of %i in %i\n", len, size );
		len = size;
	}
